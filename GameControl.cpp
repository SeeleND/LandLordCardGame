...
GameControl::GameControl(QObject *parent) :
    QObject(parent)
{
    qRegisterMetaType<CardSeq>("Hand");
...
void GameControl::Init()
{
    ...
    connect(m_user, &UserPlayer::NotifyCallLord, this, &GameControl::OnPlayerCalledLord);
    ...
    connect(m_user, &UserPlayer::NotifyPlayHand, this, &GameControl::OnPlayerPlayedHand);
    ...
    connect(m_user, &UserPlayer::NotifyPickCards, this, &GameControl::OnPlayerPickCards);
    ...
    connect(this, &GameControl::NotifyPlayerPunch, m_user, &UserPlayer::OnPlayerPunch);
...
{
    emit NotifyPlayerCalledLord(player, bet);
    ...
    if (m_betList.size() == 3)	// 三人都叫了一遍
    {
        ...
        else
        {
            ...
            QTimer::singleShot(1000, this, SLOT(StartPlayHand()));//延迟一下，开始出牌过程
        }
...
void GameControl::OnPlayerPlayedHand(Player* player, const Cards& cards)
{
    ...
    // 有炸，底分翻倍
    Type handType = CardSeq(cards).GetType();
    if (handType == Bomb || handType == Bomb_Jokers)
    {
        m_curBet = m_curBet * 2;
    }
...
{
    emit NotifyPlayerPickCards(player, cards);
}
